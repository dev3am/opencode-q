...
import { touchHeartbeat, markInstanceOffline } from "./registry"
import { migrateLegacyDefault } from "./storage"
import { createRequestHandler, tryListen, isPortInUse, isOpencodeQHost, runSweep } from "./server"
import { DEFAULT_PORT, HEARTBEAT_MS, REELECTION_MS, SWEEP_MS, isExcludedBaseDir } from "./constants"
...
export function createPluginRuntime(deps: PluginRuntimeDeps) {
  ...
  function seeSession(sid: string, meta?: { title?: string; createdAt?: string; updatedAt?: string }) {
    if (!registerable) return
  ...
  return {
    ...
    event: async ({ event }: any) => {
      if (event?.type === "server.instance.disposed") {
        if (!registerable) return
        if (event.properties?.directory === baseDir) markInstanceOffline(instanceId)
        return
      }
...
export interface DiscoverOpts { attempts?: number; delayMs?: number; log?: (msg: string) => void }

const sleep = (ms: number) => new Promise<void>((r) => setTimeout(r, ms))

export async function discoverSessions(
  client: any,
  seeSession: (id: string, meta?: { title?: string; createdAt?: string; updatedAt?: string }) => void,
...
export const OpenCodeQ = (async ({ client, directory }: any) => {
  ...
  const runtime = createPluginRuntime({ baseDir, instanceId, executor, registerable }); // prettier-ignore
  ...
  let server: http.Server | null = await tryListen(DEFAULT_PORT, handler as http.RequestListener)
  ...
  setInterval(() => {
    if (server) { try { runSweep() } catch { /* host sweep best-effort */ } }
  }, SWEEP_MS)
