...
import type { ItemStatus, QueueData, QueueItem } from "./types";
...
export function sendItem(
	...
	if (
		data.items.some(
			(i) => (i.status === "pending" || i.status === "sent") && i.id !== id,
		)
	)
		return null;
	const item = data.items.find((i) => i.id === id);
...
export function inFlightItem(
	baseDir: string,
	sessionId: string,
): QueueItem | undefined {
	return loadQueue(baseDir, sessionId).items.find(
...
export function migrateLegacyDefault(
	...
	renameSync(
		queueFile(baseDir, "default"),
		`${queueFile(baseDir, "default")}.migrated`,
	);
	return true;
}

const LIVE_STATUSES: ReadonlySet<ItemStatus> = new Set([
	"queued",
	"pending",
	"sent",
	"failed",
...
export function hasLiveItems(baseDir: string, sessionId: string): boolean {
	return loadQueue(baseDir, sessionId).items.some((i) =>
		isLiveStatus(i.status),
	);
}

export function failTimedOutPending(
	baseDir: string,
	sessionId: string,
	timeoutMs: number,
	now: number = Date.now(),
...
export function failAllInFlight(
	baseDir: string,
	sessionId: string,
	reason: string,
): boolean {
