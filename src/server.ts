...
import { HEALTH_SIGNATURE, PENDING_TIMEOUT_MS, STALE_CLEANUP_MS } from "./constants"
...
function buildState() {
  ...
  const projects = groups
    ...
    .filter((p) => p.online || p.sessions.some((s) => s.items.some((i) => S.isLiveStatus(i.status))))
  return { projects }
}
...
export function runSweep(now: number = Date.now()): void {
  ...
  for (const g of groups) {
    for (const s of g.sessions) {
      if (g.online) S.failTimedOutPending(g.baseDir, s.sessionId, PENDING_TIMEOUT_MS, now)
      else S.failAllInFlight(g.baseDir, s.sessionId, "인스턴스가 오프라인입니다")
    }
  }
  for (const rec of records) {
    ...
    const anyLive = rec.sessions.some((s) => S.hasLiveItems(rec.baseDir, s.sessionId))
...
export function createRequestHandler(opts: { webDir?: string }) {
  ...
  function serveStatic(urlPath: string, res: http.ServerResponse): boolean {
    ...
    if (!resolved.startsWith(resolve(webDir) + sep) && resolved !== resolve(webDir)) return false
    let file = resolved
  ...
  return async function handler(req: http.IncomingMessage, res: http.ServerResponse) {
    try {
      const url = new URL(req.url || "/", "http://localhost")
      const p = url.pathname

      if (p === "/health" && req.method === "GET") return send(res, 200, { app: HEALTH_SIGNATURE, version: VERSION })
      ...
      if (m && req.method === "POST") {
        ...
        const group = R.groupByBaseDir(R.readAllProjects()).find((g) => g.baseDir === baseDir)
        if (!group || !group.online) return send(res, 409, { error: "인스턴스가 오프라인입니다" })
        const item = S.sendItem(baseDir, sid, id)
        if (!item) return send(res, 409, { error: "이미 전송 중인 항목이 있습니다" })
      ...
      if (m && req.method === "PATCH") {
        ...
        try {
          const body = await readBody(req)
          const items = S.reorderItems(decodeURIComponent(b), decodeURIComponent(s), body.from, body.to)
      ...
      if (m && req.method === "POST") {
        ...
        try {
          const body = await readBody(req)
          return send(res, 201, { item: S.addItem(decodeURIComponent(b), decodeURIComponent(s), body.text) })
        } catch (e: any) { return send(res, 400, { error: e.message }) }
      }
...
export async function tryListen(port: number, handler: http.RequestListener): Promise<http.Server | null> {
