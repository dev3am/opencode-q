...
function tmp(): string { return mkdtempSync(join(tmpdir(), "oq-store-")) }
...
test("addItem creates a queued item and persists atomically", () => {
  ...
  try {
    ...
    expect(readdirSync(join(d, ".opencode")).some((f) => f.endsWith(".tmp"))).toBe(false)
...
test("loadQueue defaults a missing status to queued and survives corrupt json", () => {
  ...
  try {
    ...
    writeFileSync(join(d, ".opencode", "queue-ses1.json"), JSON.stringify({ items: [{ id: "x", text: "t", createdAt: "2026-01-01T00:00:00.000Z" }], updatedAt: "2026-01-01T00:00:00.000Z" }))
...
test("setStatus transitions and records timestamps/error", () => {
  ...
  try {
    ...
    S.setStatus(d, "ses1", item.id, "sent", { sentAt: "2026-01-01T00:00:01.000Z" })
...
test("reorderItems moves by 0-based index", () => {
  const d = tmp()
  try {
    S.addItem(d, "ses1", "a"); S.addItem(d, "ses1", "b"); S.addItem(d, "ses1", "c")
...
test("migrateLegacyDefault moves queue-default.json to the real session once", () => {
  ...
  try {
    mkdirSync(join(d, ".opencode"), { recursive: true })
    writeFileSync(join(d, ".opencode", "queue-default.json"), JSON.stringify({ items: [{ id: "old", text: "legacy", createdAt: "2026-01-01T00:00:00.000Z" }], updatedAt: "2026-01-01T00:00:00.000Z" }))
...
test("failTimedOutPending fails only pending items older than the timeout", () => {
  ...
  try {
    ...
    data.items[0].pendingAt = new Date(now - PENDING_TIMEOUT_MS - 1000).toISOString()
...
test("failTimedOutPending leaves fresh pending alone", () => {
  ...
  try {
    ...
    S.sendItem(d, "ses1", a.id)
    const changed = S.failTimedOutPending(d, "ses1", PENDING_TIMEOUT_MS, Date.now())
...
test("hasLiveItems true when queued/pending/sent/failed present, false for done/empty", () => {
  ...
  try {
    ...
    S.setStatus(d, "ses1", a.id, "done", { completedAt: new Date().toISOString() })
    expect(S.hasLiveItems(d, "ses1")).toBe(false)
  } finally { rmSync(d, { recursive: true, force: true }) }
})
