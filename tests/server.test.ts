			...
			(res) => {
				...
				res.on("end", () => {
					...
					try {
						...
						resolve({ status: res.statusCode!, json: t });
					}
				});
			},
		);
...
test("send marks the item pending; a second send returns 409", async () => {
	...
	expect(
		S.loadQueue(proj, "ses1").items.find((i) => i.id === add.json.item.id)!
			.status,
	).toBe("pending");
...
test("two instances of one baseDir collapse to a single project", async () => {
	...
	expect(matches[0].sessions.map((s: any) => s.sessionId).sort()).toEqual([
		"ses1",
		"ses2",
	]);
});
...
test("runSweep fails timed-out pending in an ONLINE project", () => {
	...
	data.items[0].pendingAt = new Date(
		Date.now() - PENDING_TIMEOUT_MS - 1000,
	).toISOString();
...
test("runSweep fails all in-flight items in an OFFLINE project", () => {
	...
	R.writeProjectRecord({
		baseDir: proj,
		sessions: [{ sessionId: "ses1", status: "idle" }],
		heartbeat: new Date(Date.now() - 999999).toISOString(),
		instanceId: "i1",
	});
	R.writeProjectRecord({
...
test("runSweep removes a long-stale instance record with no live items", () => {
	R.writeProjectRecord({
		baseDir: proj,
		sessions: [{ sessionId: "ses1", status: "idle" }],
		heartbeat: new Date(Date.now() - STALE_CLEANUP_MS - 1000).toISOString(),
		instanceId: "dead",
	});
	runSweep(Date.now());
