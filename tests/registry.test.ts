...
let home: string
beforeEach(() => { home = mkdtempSync(join(tmpdir(), "oq-reg-")); process.env.OPENCODE_Q_HOME = home })
afterEach(() => { delete process.env.OPENCODE_Q_HOME; rmSync(home, { recursive: true, force: true }) })
...
test("groupByBaseDir merges sessions and is online if any instance is fresh", () => {
  const now = Date.now()
  R.writeProjectRecord({ baseDir: "/p/a", sessions: [{ sessionId: "s1", status: "idle" }], heartbeat: new Date(now).toISOString(), instanceId: "i1" })
  R.writeProjectRecord({ baseDir: "/p/a", sessions: [{ sessionId: "s2", status: "idle" }], heartbeat: new Date(now - LIVENESS_TIMEOUT_MS - 5000).toISOString(), instanceId: "i2" })
  const groups = R.groupByBaseDir(R.readAllProjects(), now)
  expect(groups).toHaveLength(1)
  expect(groups[0].online).toBe(true)
  expect(groups[0].sessions.map((s) => s.sessionId).sort()).toEqual(["s1", "s2"])
})

test("groupByBaseDir dedupes same sessionId keeping the latest updatedAt", () => {
  const now = Date.now()
  R.writeProjectRecord({ baseDir: "/p/a", sessions: [{ sessionId: "s1", status: "idle", title: "old", updatedAt: new Date(now - 10000).toISOString() }], heartbeat: new Date(now).toISOString(), instanceId: "i1" })
  R.writeProjectRecord({ baseDir: "/p/a", sessions: [{ sessionId: "s1", status: "busy", title: "new", updatedAt: new Date(now).toISOString() }], heartbeat: new Date(now).toISOString(), instanceId: "i2" })
...
test("isOnline true for fresh, false past the timeout", () => {
  const now = Date.now()
  expect(R.isOnline({ baseDir: "/a", sessions: [], heartbeat: new Date(now).toISOString(), instanceId: "i" }, now)).toBe(true)
  expect(R.isOnline({ baseDir: "/a", sessions: [], heartbeat: new Date(now - LIVENESS_TIMEOUT_MS - 1000).toISOString(), instanceId: "i" }, now)).toBe(false)
})
