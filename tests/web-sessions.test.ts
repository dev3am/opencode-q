...
import { sortSessionsByRecent, formatRelativeTime } from "../web/src/lib/sessions"

test("sortSessionsByRecent orders by updatedAt desc, undefined last", () => {
  const a = { sessionId: "a", status: "idle", updatedAt: "2026-05-26T10:00:00.000Z" }
  const b = { sessionId: "b", status: "idle", updatedAt: "2026-05-26T12:00:00.000Z" }
  const c = { sessionId: "c", status: "idle" }
  expect(sortSessionsByRecent([a, b, c] as any).map((s) => s.sessionId)).toEqual(["b", "a", "c"])
})
