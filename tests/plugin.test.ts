...
beforeEach(() => { home = mkdtempSync(join(tmpdir(), "oq-plug-home-")); proj = mkdtempSync(join(tmpdir(), "oq-plug-proj-")); process.env.OPENCODE_Q_HOME = home })
afterEach(() => { delete process.env.OPENCODE_Q_HOME; rmSync(home, { recursive: true, force: true }); rmSync(proj, { recursive: true, force: true }) })
...
test("a session event registers the session in the registry", async () => {
  ...
  await rt.event({ event: { type: "session.idle", properties: { sessionID: "ses1" } } })
...
test("session.error marks the in-flight item failed", async () => {
  ...
  const item = S.addItem(proj, "ses1", "x")
  S.setStatus(proj, "ses1", item.id, "sent")
  await rt.event({ event: { type: "session.error", properties: { sessionID: "ses1", error: { name: "ProviderError" } } } })
...
test("first sight of a session migrates legacy queue-default.json", async () => {
  ...
  const { rt } = makeRuntime()
  await rt.event({ event: { type: "session.idle", properties: { sessionID: "ses-real" } } })
  expect(S.loadQueue(proj, "ses-real").items.map((i) => i.text)).toEqual(["legacy"])
})
...
test("an excluded baseDir (home/root) does NOT write a registry record", async () => {
  ...
  const rt = createPluginRuntime({ baseDir: proj, instanceId: "i1", executor, registerable: false })
  await rt.event({ event: { type: "session.idle", properties: { sessionID: "ses1" } } })
  rt.persist()
...
test("a registerable runtime DOES write a record", async () => {
  const executor = createExecutor({ baseDir: proj, prompt: async () => {} })
  const rt = createPluginRuntime({ baseDir: proj, instanceId: "i1", executor, registerable: true })
  await rt.event({ event: { type: "session.idle", properties: { sessionID: "ses1" } } })
...
test("session.updated captures title and times into the registry", async () => {
  const { rt } = makeRuntime()
  await rt.event({ event: { type: "session.updated", properties: { info: {
    id: "ses1", title: "Fix the parser", parentID: undefined,
    time: { created: 1_700_000_000_000, updated: 1_700_000_500_000 },
  } } } })
  const s = R.readAllProjects()[0].sessions.find((x) => x.sessionId === "ses1")!
...
test("child sessions (with parentID) are NOT registered", async () => {
  const { rt } = makeRuntime()
  await rt.event({ event: { type: "session.created", properties: { info: {
    id: "child1", title: "subagent", parentID: "ses1",
    time: { created: 1, updated: 2 },
  } } } })
  expect(R.readAllProjects().some((r) => r.sessions.some((s) => s.sessionId === "child1"))).toBe(false)
})
...
test("server.instance.disposed marks this instance offline immediately", async () => {
  ...
  await rt.event({ event: { type: "server.instance.disposed", properties: { directory: proj } } })
...
test("discoverSessions retries on failure then succeeds", async () => {
  ...
  const client = {
    session: {
      list: async () => { calls++; if (calls < 2) throw new Error("boom"); return { data: [{ id: "ses1", title: "t", time: { created: 1, updated: 2 } }] } },
      status: async () => ({ data: {} }),
    },
  }
  const { discoverSessions } = await import("../src/plugin")
  const ok = await discoverSessions(client as any, (id: string, meta: any) => seen.push(id), () => {}, { attempts: 3, delayMs: 0 })
  expect(ok).toBe(true)
