...
let d: string
beforeEach(() => { d = mkdtempSync(join(tmpdir(), "oq-exec-")) })
...
test("tick dispatches a pending item when the session is idle, then onIdle marks it done", async () => {
  ...
  const exec = createExecutor({ baseDir: d, prompt: async (sid, text) => { sent.push({ sid, text }) } })
...
test("tick does not dispatch while the session is busy", async () => {
  const sent: string[] = []
  const exec = createExecutor({ baseDir: d, prompt: async (_s, t) => { sent.push(t) } })
...
test("a prompt rejection marks the item failed", async () => {
  const exec = createExecutor({ baseDir: d, prompt: async () => { throw new Error("connection refused") } })
  exec.noteSession("ses1", "idle")
  const item = S.addItem(d, "ses1", "x")
  S.setStatus(d, "ses1", item.id, "pending")
  await exec.tick()
