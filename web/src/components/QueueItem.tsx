  ...
  return (
    <div className="flex items-start gap-2 group py-1.5">
      <div className="flex flex-col items-center pt-3 gap-1 min-w-[18px]">
        <span {...dragHandleProps} className="cursor-grab text-gray-300 select-none hover:text-gray-500 transition-colors text-sm leading-none" title={t("queue.dragReorder")}>≡</span>
      ...
      {editing ? (
        <div className="flex-1 flex items-start gap-1">
          ...
          />
          <button onClick={() => onRemove(item.id)} className="bg-transparent border-none cursor-pointer text-gray-300 hover:text-red-400 text-xs pt-3 transition-colors" title={t("queue.remove")}>✕</button>
        </div>
      ) : (
        ...
        >
          ...
          {onExecute && (
            <button
              ...
              title={t("queue.execute")}
            >▶</button>
          )}
          {!editing && (
            <div className="absolute bottom-1.5 right-1.5 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={(e) => { e.stopPropagation(); setEditing(true) }} className="bg-transparent border-none cursor-pointer text-gray-300 hover:text-gray-500 text-xs transition-colors" title={t("queue.edit")}>✎</button>
              <button onClick={(e) => { e.stopPropagation(); onRemove(item.id) }} className="bg-transparent border-none cursor-pointer text-gray-300 hover:text-red-400 text-xs transition-colors" title={t("queue.remove")}>✕</button>
            </div>
          )}
        </div>
      )}
