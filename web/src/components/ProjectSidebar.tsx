...
import { sortSessionsByRecent, formatRelativeTime } from "../lib/sessions"
...
export default function ProjectSidebar({ projects, selectedBaseDir, selectedSessionId, onSelectProject, onSelectSession }: Props) {
  ...
  return (
    <div className="flex flex-col w-72 shrink-0 bg-gray-50 h-screen overflow-y-auto">
      <div className="sticky top-0 z-10 bg-gray-50/90 backdrop-blur-sm px-3 pt-3 pb-2">
        <div className="text-[11px] uppercase tracking-wider text-gray-400 font-semibold">Projects</div>
      </div>
            ...
            >
              ...
              >
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${p.online ? "bg-green-500" : "bg-gray-400"}`} />
              ...
              {isActiveProject && p.sessions.length > 0 && (
                <div className="max-h-48 overflow-y-auto px-2 pb-2">
                  {sortSessionsByRecent(p.sessions).map((s) => {
                    ...
                    const label = (s.title && s.title.trim()) ? s.title : s.sessionId.slice(0, 8)
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${STATUS_DOT[s.status] || "bg-gray-300"}`} />
                          <span className="truncate flex-1" title={s.title || s.sessionId}>{label}</span>
                        </div>
                        {when && <div className="text-[10px] text-gray-400 mt-0.5 pl-3.5">{when}</div>}
                      </div>
                    )
                  })}
                </div>
